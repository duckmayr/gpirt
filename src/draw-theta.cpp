...
inline double compute_ll(const double nu,
                     const arma::vec& y,
...
inline arma::vec draw_theta_ess(const arma::mat& y,
                     const arma::mat& L,
    ...
    {
        log_y += compute_ll(nu(h), y.col(h), fstar.slice(h),
    ...
    arma::vec nu_prime(horizon, arma::fill::zeros);
    int iter = 0;

    while ( reject ) {
        iter += 1;
        // Get nu_prime given current epsilon
        nu_prime = nu * std::cos(epsilon) + nu * std::sin(epsilon);
        nu_prime.clamp(-5.0, 5.0);
        // If the log likelihood is over our threshold, accept
        double log_y_prime = 0;
        for (arma::uword h = 0; h < horizon; h++)
        {
            log_y_prime += compute_ll(nu_prime(h), y.col(h),
...
arma::mat draw_theta(const arma::vec& theta_star,
    ...
    if(ls>=3*horizon){
        ...
        for ( arma::uword i = 0; i < n; ++i ){
            ...
            arma::vec raw_theta_ess = draw_theta_ess(y_, V, fstar_, mu_star_, thresholds);
            for ( arma::uword h = 0; h < horizon; ++h ){
        ...
        for ( arma::uword i = 0; i < n; ++i ){
            ...
            for ( arma::uword h = 0; h < horizon; ++h ){
                ...
                arma::vec raw_theta_ess = draw_theta_ess(y_, V, fstar_,
                            mu_star_, thresholds);
                result(i, h) = theta_star[round((raw_theta_ess(0)+5)/0.01)];
            }
        }
        ...
        for ( arma::uword i = 0; i < n; ++i ){
            ...
            arma::vec raw_theta_ess = draw_theta_ess(y.row(i), L, fstar, mu_star, thresholds);
            // round up theta to nearest fine grid
            for ( arma::uword h = 0; h < horizon; ++h ){
