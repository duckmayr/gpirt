...
// For convenience, I include all the same arguments to each function,
// though they don't all need all of them. So, this ignores things for me:
void ignore(arma::mat x) {}

// Mean zero, store f* draws
...
Rcpp::List gpirtMCMC0(const arma::mat& y, arma::vec theta,
    ...
    ignore(beta_prior_means);
    ignore(beta_prior_sds);
    ignore(beta_step_sizes);
    int n = y.n_rows;
    int m = y.n_cols;
    ...
    arma::mat f_star(N, m);
    // Set up the mean additions (in this case 0)
    arma::mat mu = arma::zeros<arma::mat>(n, m);
    arma::mat mu_star = arma::zeros<arma::mat>(N, m);
    ...
    for ( int iter = 0; iter < total_iterations; ++iter ) {
        ...
        Rprintf("\r%7.3f %% complete", progress);
...
// Mean zero, do not store f* draws
...
// Linear mean, store f* draws
// [[Rcpp::export(.gpirtMCMC2)]]
Rcpp::List gpirtMCMC2(const arma::mat& y, arma::vec theta,
                      const int sample_iterations, const int burn_iterations,
                      const arma::vec& means, const arma::uvec& groups,
                      const double sf, const double ell,
                      const arma::mat& beta_prior_means,
...
// Linear mean, do not store f* draws
// [[Rcpp::export(.gpirtMCMC3)]]
Rcpp::List gpirtMCMC3(const arma::mat& y, arma::vec theta,
                      const int sample_iterations, const int burn_iterations,
                      const arma::vec& means, const arma::uvec& groups,
                      const double sf, const double ell,
                      const arma::mat& beta_prior_means,
...
// Quadratic mean, store f* draws
// [[Rcpp::export(.gpirtMCMC4)]]
Rcpp::List gpirtMCMC4(const arma::mat& y, arma::vec theta,
                      const int sample_iterations, const int burn_iterations,
                      const arma::vec& means, const arma::uvec& groups,
                      const double sf, const double ell,
                      const arma::mat& beta_prior_means,
    ...
    Xstar.col(0) = arma::ones<arma::vec>(N);
    Xstar.col(1) = theta_star;
    Xstar.col(2) = arma::square(theta_star);
    arma::mat mu_star = Xstar * beta;
    // The prior probabilities for theta_star doesn't change between iterations
    int num_groups = groups.n_elem;
    arma::mat theta_prior(N, num_groups);
    ...
    // Setup results storage
    arma::mat theta_draws(sample_iterations + 1, n);
    arma::cube beta_draws(3, m, sample_iterations + 1);
    arma::cube f_draws(n, m, sample_iterations + 1);
    arma::cube fstar_draws(N, m, sample_iterations + 1);
    // Store initial values
    theta_draws.row(0)   = theta.t();
...
// Quadratic mean, do not store f* draws
// [[Rcpp::export(.gpirtMCMC5)]]
Rcpp::List gpirtMCMC5(const arma::mat& y, arma::vec theta,
                      const int sample_iterations, const int burn_iterations,
                      const arma::vec& means, const arma::uvec& groups,
                      const double sf, const double ell,
                      const arma::mat& beta_prior_means,
