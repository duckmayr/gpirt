...
arma::mat draw_beta(const arma::mat& beta, const arma::mat& X,
                    ...
                    const arma::mat& step_sizes, const arma::mat& thresholds) {
    ...
    for ( arma::uword j = 0; j < m; ++j ) {
        ...
        for ( arma::uword k = 0; k < p; ++k ) {
            ...
            double pv_ll      = ll_bar(rho, responses, X * pv, thresholds.row(j).t());
            double cv_ll      = ll_bar(rho, responses, X * cv, thresholds.row(j).t());
